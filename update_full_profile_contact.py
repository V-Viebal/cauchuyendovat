...
# Add CSS for detailed contact card & b2b action buttons
contact_css = '''
/* Detailed B2B Partner Contact Box */
.b2b-contact-card {
  background: #ffffff;
...
text = text.replace('</style>', contact_css + '\n</style>')

# Replace profileData dictionary with comprehensive contact details for each role
full_profile_data_js = '''const profileData = {
  'designer_ind': {
...
# Replace profileData in text
start_idx = text.find('const profileData = {')
end_idx = text.find('function switchProfile(', start_idx)
text = text[:start_idx] + full_profile_data_js + '\n\n' + text[end_idx:]

with open('mirrored_pages/profile.html', 'w', encoding='utf-8') as f:
    f.write(text)

print("Updated profile.html with detailed partner contact boxes for all roles!")
