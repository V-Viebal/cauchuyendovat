...
# Let's inspect the two scripts in profile.html:
# Script 0 initializes profile and runs on DOMContentLoaded.
# Script 1 (syncAuthHeader) runs independently and might overwrite header with old/wrong localStorage data!
# In fact, let's look closely at why the screenshot showed:
# The header button showed: "TAVICO Timber" (Nhà cung cấp vật liệu)
# BUT the URL was: http://localhost:3147/profile?role=designer_ind !
# The page rendered: Huỳnh Lê Phương Uyên (Designer cá nhân)!
# Because earlier, the user clicked "TAVICO Timber (Nhà cung ứng)", which set localStorage to TAVICO Timber.
# Then the user navigated or tested /profile?role=designer_ind.
# On the page, the profile rendered role=designer_ind (Huỳnh Lê Phương Uyên).
# BUT the header syncAuthHeader read localStorage (which still had TAVICO Timber)!
# Or vice versa: if the user wants to log in as TAVICO Timber, they should see TAVICO Timber both on the header AND on the profile!

# Let's fix this so that:
# 1. When ?role=... is present in the URL, localStorage is IMMEDIATELY updated to that user BEFORE anything else runs, and the header reflects that EXACT user!
# 2. When quickLogin in login.html is clicked, role AND name are stored synchronously, and redirect goes to /profile?role=...
# 3. Inside profile.html, we unify the auth sync and profile render so they NEVER get out of sync!

unified_script = '''
<script>
// Master Profile Data & Synced State
const profileData = {
  'designer_ind': {
...
(function(){
  ...
  // URL parameter has absolute priority
  ...
  // Set localStorage immediately before DOM rendering
  try {
    const d = profileData[targetRole];
    localStorage.setItem('monos_current_user', JSON.stringify({
      role: targetRole,
      name: d.name,
      email: d.email,
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => switchProfile(targetRole));
  } else {
...
# Remove old scripts at the end of profile.html and replace with unified_script
idx_scr = text.find('<script>')
text = text[:idx_scr] + unified_script + '</body>\n</html>'

with open('mirrored_pages/profile.html', 'w', encoding='utf-8') as f:
    f.write(text)

print("Unified profile script and state synchronization successfully!")
